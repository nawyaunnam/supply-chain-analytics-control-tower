...
def export(root, target="local"):
    ...
    source_kind = (
        json.loads(context_path.read_text()).get("source_kind", "unclassified")
    ...
    cards = "".join(
        ...
        for label, value in [
            ...
            ("First-dispatch fill rate", percentage(fill)),
            ("Stockout SKU-days", f"{stockout:.1%}"),
            ("Forecast horizon", "14 days"),
        ]
    )
    riskrows = "".join(
        f"<tr><td>{html.escape(str(r['sku']))}</td><td>{html.escape(str(r['warehouse_id']))}</td><td>{r['available_qty']}</td><td>{fmt(r['next_14_day_demand'])}</td><td>{fmt(r['projected_shortfall'])}</td></tr>"
        for r in risk
    )
    alertrows = "".join(
        f"<tr><td>{r['date_day']}</td><td>{html.escape(str(r['warehouse_id']))} / {html.escape(str(r['sku']))}</td><td>{r['metric'].replace('_', ' ')}</td><td>{fmt(r['robust_score'])}</td></tr>"
        for r in alerts
    )
    supplierrows = "".join(
        f"<tr><td>{html.escape(str(r['supplier_id']))}</td><td>{float(r['otif_rate']):.1%}</td><td>{fmt(r['average_lead_days'])}</td><td>{r['overdue_unreceived']}</td></tr>"
    ...
    chosen = sorted({(r["sku"], r["warehouse_id"]) for r in data["fct_demand_daily"]})[0]
    series_label = html.escape(" / ".join(chosen))
    series = sorted(
        [r for r in data["fct_demand_daily"] if (r["sku"], r["warehouse_id"]) == chosen],
        key=lambda r: r["date_day"],
    )[-42:]
    future = sorted(
        [r for r in data["fct_forecasts"] if (r["sku"], r["warehouse_id"]) == chosen],
...
<main><header><small>SUPPLY CHAIN / CONTROL TOWER</small><h1>See the delay. Anticipate the shortage.</h1><div class="muted">Orders · inventory · suppliers · transportation | {html.escape(source_kind.capitalize())} data as of {summary["as_of"]}</div></header>
<section class="cards">{cards}</section><section class="panel"><h2>Demand outlook · {series_label}</h2><div class="muted">Last 42 days + 14-day forecast · green: observed · blue: forecast · shaded: empirical error band</div><svg viewBox="0 0 880 235" role="img" aria-label="Historical demand and forecast"><polygon points="{upper} {lower}" fill="#588dcd" opacity=".2"/><polyline points="{history}" stroke="#66dcc1" stroke-width="2" fill="none"/><polyline points="{predicted}" stroke="#72aaff" stroke-width="3" fill="none"/></svg></section>
